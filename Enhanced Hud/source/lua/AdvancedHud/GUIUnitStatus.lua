...
local function UpdateItemsGUIScale()
    ...
    -- For WeaponDisplay
    kClipBarWidth = GUIScale(130)
    kClipBarHeight = GUIScale(4)
...
local function CreateBlipItem(self)
    ...
    WD_addToBlip(newBlip) -- For WeaponDisplay
...
local function UpdateBlipTeamType(blip)
    ...
    blip.WeaponClipBar:SetColor(kArmorBarColors[teamType]) -- For WeaponDisplay
...
function GUIUnitStatus:UpdateUnitStatusBlip(blipIndex, localPlayerIsCommander, baseResearchRot, showHints, playerTeamType )
    ...
    local clipFractionInWeapon = false -- For WeaponDisplay
    local weaponColor = false -- For WeaponDisplay
    ...
    if isCrosshairTarget or commHealthBarsShown then
        ...
        clipFractionInWeapon = blipData.ClipFractionInWeapon -- For WeaponDisplay
        weaponColor = blipData.WeaponColor -- For WeaponDisplay
        alpha = 1
    end
    ...
    WD_WeaponClipBar(alpha, localPlayerIsCommander, blipData, updateBlip, isEnemy, clipFractionInWeapon, hideBars, teamType, weaponColor)  -- For WeaponDisplay
...
local function FindUnitsToDisplayStatusFor(player)
    ...
    if crosshairTarget
            ...
            and crosshairTarget:GetIsSelectable(teamNumber))  or player:isa("Marine")) then -- For WeaponDisplay 
                -- Marines can see them even if they are not selectable
...
--For WeaponDisplay
function WD_addToBlip(newBlip)

    local neutralTexture = "ui/unitstatus_neutral.dds"
    local teamType = PlayerUI_GetTeamType()


    newBlip.WeaponClipBarBg = GetGUIManager():CreateGraphicItem()
    newBlip.WeaponClipBarBg:SetAnchor(GUIItem.Middle, GUIItem.Bottom)
    newBlip.WeaponClipBarBg:SetSize(Vector(kClipBarWidth, kClipBarHeight * 1.5, 0))
    newBlip.WeaponClipBarBg:SetPosition(Vector(-kClipBarWidth / 2, -kClipBarHeight - GUIScale(10), 0)) -- 
    newBlip.WeaponClipBarBg:SetTexture(neutralTexture)
    newBlip.WeaponClipBarBg:SetColor(Color(0,0,0,0))
    newBlip.WeaponClipBarBg:SetTexturePixelCoordinates(GUIUnpackCoords(GUIUnitStatus.kUnitStatusBarTexCoords))
    
    newBlip.WeaponClipBar = GUIManager:CreateGraphicItem()
    newBlip.WeaponClipBar:SetColor(kArmorBarColors[teamType])
    newBlip.WeaponClipBar:SetSize(Vector(kClipBarWidth, kClipBarHeight * 1.5, 0))
    newBlip.WeaponClipBar:SetTexture(neutralTexture)
    newBlip.WeaponClipBar:SetTexturePixelCoordinates(GUIUnpackCoords(GUIUnitStatus.kUnitStatusBarTexCoords))
    newBlip.WeaponClipBar:SetBlendTechnique(GUIItem.Add)
    newBlip.WeaponClipBarBg:AddChild(newBlip.WeaponClipBar)

    newBlip.statusBg:AddChild(newBlip.WeaponClipBarBg)

    return newBlip
end

--For WeaponDisplay
function WD_WeaponClipBar(alpha, localPlayerIsCommander, blipData, updateBlip, isEnemy, clipFractionInWeapon, hideBars, teamType, weaponColor)

    -- option has to be checked at least once. Option changes to wd_enabled will be directly applied in modsmenudata
    if wd_enabled == nil then 
        wd_enabled = Client.GetOptionBoolean("wd_enabled", true)
    end
    if wd_comm_enabled == nil then 
        wd_comm_enabled = Client.GetOptionBoolean("wd_comm_enabled", true)
    end

    local displayWABar = alpha > 0 and not hideBars and not isEnemy

    if not displayWABar or (not wd_comm_enabled and localPlayerIsCommander) or (not wd_enabled and not localPlayerIsCommander) then
        updateBlip.WeaponClipBarBg:SetIsVisible(false)
        updateBlip.WeaponClipBar:SetIsVisible(false)
        return
    end


 
    --case: blip is a weapon
    if not blipData.IsPlayer and blipData.WeaponColor then -- weapon on the ground
        ...
        if blipData.WeaponColor == Color(1,1,1,1) then -- case welder, dont show clip bar
            
            updateBlip.WeaponClipBarBg:SetIsVisible(false)
            updateBlip.WeaponClipBar:SetIsVisible(false)

        elseif clipFractionInWeapon then -- other weapons

            updateBlip.WeaponClipBarBg:SetIsVisible(true)
            updateBlip.WeaponClipBar:SetIsVisible(true)


            updateBlip.WeaponClipBarBg:SetColor(kArmorBarBgColors[teamType])
            updateBlip.WeaponClipBar:SetColor(blipData.WeaponColor)

            updateBlip.WeaponClipBar:SetSize(Vector(kClipBarWidth * clipFractionInWeapon, kClipBarHeight * 1.5 , 0))
            updateBlip.WeaponClipBar:SetTexturePixelCoordinates(GetPixelCoordsForFraction(clipFractionInWeapon))
        end


    --case: player is the commander, blip is a player
    elseif localPlayerIsCommander and blipData.IsPlayer and clipFractionInWeapon then -- commander can also see clipsizes on marines

        updateBlip.WeaponClipBarBg:SetIsVisible(true)
        updateBlip.WeaponClipBar:SetIsVisible(true)

        updateBlip.WeaponClipBarBg:SetColor(Color(0,0,0,1)) -- black
        updateBlip.WeaponClipBar:SetColor(Color(0 ,0, 1,1)) -- blue

        updateBlip.WeaponClipBar:SetSize(Vector(kClipBarWidth * clipFractionInWeapon, kClipBarHeight * 1.5 , 0))
        updateBlip.WeaponClipBar:SetTexturePixelCoordinates(GetPixelCoordsForFraction(clipFractionInWeapon))

        -- move it slightly to be below the ammo bar
        updateBlip.WeaponClipBarBg:SetPosition(Vector(-kClipBarWidth / 2, kClipBarHeight * 2.5 - GUIScale(10), 0))
    else 
