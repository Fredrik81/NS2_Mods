...
GUIScoreboard.kTeamSmallFont = ReadOnly {
    family = "Arial",
    size = 10
}
GUIScoreboard.kTeamSmallBoldFont = ReadOnly {
    family = "Arial",
    size = 11
}
GUIScoreboard.kTeamLargeFont = ReadOnly {
    family = "Arial",
    size = 12
}
GUIScoreboard.kTeamLargeBoldFont = ReadOnly {
    family = "Arial",
    size = 13
}
...
local function fetchPlayerStats(fetchTable)
    ...
    if usersToFetch then
        ...
        Shared.SendHTTPRequest(string.format(fetchUrl, usersToFetch), "GET", {}, function(data)
            if not data then
                return
            end
...
local function ResolveLocalizedString(key)
    if not key then
        return ""
    end

    local cachedValue = kResolvedStrings[key]
    if cachedValue ~= nil then
        return cachedValue
    end

    local resolvedValue = Locale.ResolveString(key)
    if resolvedValue ~= nil then
        kResolvedStrings[key] = resolvedValue
    end
...
function GUIScoreboard:GetTeamItemWidth()
    PROFILE("GUIScoreboard:GetTeamItemWidth")
...
function GUIScoreboard:Uninitialize()
    PROFILE("GUIScoreboard:Uninitialize")
...
local function SetMouseVisible(self, setVisible)
    PROFILE("GUIScoreboard:SetMouseVisible")
...
local function HandleSlidebarClicked(self, _, mouseY)
    PROFILE("GUIScoreboard:HandleSlidebarClicked")
...
function GUIScoreboard:Update_topBarMode()
    PROFILE("GUIScoreboard:Update_topBarMode")
...
function GUIScoreboard:Update_GUIElements(deltaTime, contentXSize, contentYSize)
    PROFILE("GUIScoreboard:Update_GUIElements")

    local sizeChanged = false
    if contentXSize ~= kGUIdata.contentXSize or contentYSize ~= kGUIdata.contentYSize then
        kGUIdata.contentXSize = contentXSize
        kGUIdata.contentYSize = contentYSize
        sizeChanged = true
    end
...
function GUIScoreboard:Update_toNewSizes(teamGUISize)
    PROFILE("GUIScoreboard:Update_toNewSizes")
...
function GUIScoreboard:Update_SetMainVisibles()
    PROFILE("GUIScoreboard:Update_SetMainVisibles")
...
function GUIScoreboard:UpdateTeam_CommanderName(playerObject, teamObject)
    ...
    if (playerObject.isCommander or (playerObject.isLastComm and not playerObject.isBot)) and teamObject.isPlayingTeam then
        ...
        playerObject.playerGUIItem.CommIcon:SetIsVisible(true)
    else
        playerObject.playerGUIItem.CommIcon:SetIsVisible(false)
    end
...
function GUIScoreboard:UpdateTeam_HighlightCurrentPlayer(playerObject, teamObject)
...
local function GetPlayerBadgeData(playerGUIItem, clientIndex)
    if clientIndex and playerGUIItem.badgeDataClientIndex ~= clientIndex then
        local badgeTextures, badgeNames = Badges_GetBadgeTextures(clientIndex, "scoreboard")
        playerGUIItem.badgeTextures = badgeTextures
        playerGUIItem.badgeNames = badgeNames
        playerGUIItem.badgeDataClientIndex = clientIndex
    end

    return playerGUIItem.badgeTextures or {}, playerGUIItem.badgeNames or {}
end
...
function GUIScoreboard:UpdateTeam_PlayersPlaceAndName(playerObject)
    ...
    for i = 1, #playerObject.playerGUIItem["BadgeItems"] do
        playerObject.playerGUIItem["BadgeItems"][i]:SetPosition(Vector(numberSize + kPlayerItemLeftMargin + (i - 1) * kPlayerVoiceChatIconSize + (i - 1) * kPlayerBadgeRightPadding, -kPlayerVoiceChatIconSize / 2, 0) *
    ...
    if playerStatus == "-" or (playerStatus ~= ResolveLocalizedString("STATUS_SPECTATOR") and playerObject["teamNumber"] ~= kTeam1Index and playerObject["teamNumber"] ~= kTeam2Index) then
    ...
    local badgeTextures = GetPlayerBadgeData(playerObject.playerGUIItem, playerObject["clientIndex"])
    SetPlayerItemBadges(playerObject.playerGUIItem, badgeTextures)

    local numBadges = math.min(#badgeTextures, #playerObject.playerGUIItem["BadgeItems"])
...
function GUIScoreboard:UpdateTeam_SkillIcons(playerObject)
    ...
    if skillIconOverrideSettings then
        ...
        playerObject.playerGUIItem.SkillIcon.tooltipText = string.format(ResolveLocalizedString("SKILLTIER_TOOLTIP"), ResolveLocalizedString(playerObject.playerSkillTierName), playerObject.playerSkillTier)
    ...
    if playerObject["isBot"] then
        ...
        if playerObject.isSpectator or playerObject.isLastComm or isPreGame then
            playerObject.playerGUIItem.SkillIcon.tooltipText = playerObject.playerGUIItem.SkillIcon.tooltipText .. "\nCom Tier: " .. ResolveLocalizedString(playerObject.comSkillTierName) .. " (" .. tostring(playerObject.comSkillTier) .. ")"
        end
        if playerObject.isSpectator or playerObject.isLastComm or isPreGame then
            ...
            if GUIItemContainsPoint(playerObject.playerGUIItem["Background"], mouseX, mouseY) then
                ...
                for i = 1, #playerObject.playerGUIItem["BadgeItems"] do
                    ...
                    if GUIItemContainsPoint(badgeItem, mouseX, mouseY) and badgeItem:GetIsVisible() then
                        local _, badgeNames = GetPlayerBadgeData(playerObject.playerGUIItem, playerObject["clientIndex"])
...
function GUIScoreboard:UpdateTeam_EalIcons(teamObject)
    if teamObject.isVisibleTeam and teamObject.isPlayingTeam then
        ...
        if teamObject.teamNumber == kTeam1Index then
            ...
            lifeformCount = GetCountByStatus(teamObject.teamScores, ResolveLocalizedString("STATUS_RIFLE"), true)
            ...
            lifeformCount = GetCountByStatus(teamObject.teamScores, ResolveLocalizedString("STATUS_SHOTGUN"), true)
            ...
            lifeformCount = GetCountByStatus(teamObject.teamScores, ResolveLocalizedString("STATUS_FLAMETHROWER"), true)
            ...
            lifeformCount = GetCountByStatus(teamObject.teamScores, ResolveLocalizedString("STATUS_HMG"), true)
            ...
            lifeformCount = GetCountByStatus(teamObject.teamScores, ResolveLocalizedString("STATUS_GRENADE_LAUNCHER"), true)
        ...
        elseif teamObject.teamNumber == kTeam2Index then
            lifeformCount = GetCountByStatus(teamObject.teamScores, ResolveLocalizedString("STATUS_SKULK"), true)
            ...
            lifeformCount = GetCountByStatus(teamObject.teamScores, ResolveLocalizedString("STATUS_GORGE"), true)
            ...
            lifeformCount = GetCountByStatus(teamObject.teamScores, ResolveLocalizedString("STATUS_LERK"), true)
            ...
            lifeformCount = GetCountByStatus(teamObject.teamScores, ResolveLocalizedString("STATUS_FADE"), true)
            ...
            lifeformCount = GetCountByStatus(teamObject.teamScores, ResolveLocalizedString("STATUS_ONOS"), true)
            ...
            lifeformCount = GetShellLevel(teamObject.teamNumber)
            ...
            lifeformCount = GetVeilLevel(teamObject.teamNumber)
            ...
            lifeformCount = GetSpurLevel(teamObject.teamNumber)
...
function GUIScoreboard:UpdateTeam(updateTeam)
    ...
    local teamObject = {}
    teamObject.teamGUIItem = updateTeam["GUIs"]["Background"]
    teamObject.teamNameGUIItem = updateTeam["GUIs"]["TeamName"]
    teamObject.teamSkillGUIItem = updateTeam["GUIs"]["TeamSkill"]
    teamObject.teamInfoGUIItem = updateTeam["GUIs"]["TeamInfo"]
    teamObject.teamNameText = ResolveLocalizedString(string.format("NAME_TEAM_%s", updateTeam["TeamNumber"]))
    teamObject.teamColor = updateTeam["Color"]
    teamObject.localPlayerHighlightColor = updateTeam["HighlightColor"]
    teamObject.playerList = updateTeam["PlayerList"]
    teamObject.teamScores = updateTeam["GetScores"]()
    teamObject.teamNumber = updateTeam["TeamNumber"]
    teamObject.isPlayingTeam = teamObject.teamNumber ~= kTeamReadyRoom
    ...
    -- If we are throttline GUI updates
    local throttlePerformUpdates = guiThrottleTime[teamObject.teamNumber] and Shared.GetTime() > guiThrottleTime[teamObject.teamNumber] or false
    ...
    local playersOnTeamText = string.format("%d %s", teamObject.numPlayers, teamObject.numPlayers == 1 and ResolveLocalizedString("SB_PLAYER") or ResolveLocalizedString("SB_PLAYERS"))
    ...
    if not teamObject.isPlayingTeam then
        ...
        if numPlayersConnecting > 0 then
            ...
            if teamObject.numPlayers > 0 then
                teamHeaderText = string.format("%s (%s, %d %s)", teamObject.teamNameText, playersOnTeamText, numPlayersConnecting, ResolveLocalizedString("SB_CONNECTING"))
            else
                teamHeaderText = string.format("%s (%d %s)", teamObject.teamNameText, numPlayersConnecting, ResolveLocalizedString("SB_CONNECTING"))
            end
        end
    end

    if not teamHeaderText then
        teamHeaderText = string.format("%s (%s)", teamObject.teamNameText, playersOnTeamText)
    end

    teamObject.teamNameGUIItem:SetText(teamHeaderText)

    -- Update team resource display
    if teamObject.isPlayingTeam then
        local teamResourcesString = ConditionalValue(teamObject.isVisibleTeam, string.format(ResolveLocalizedString("SB_TEAM_RES"), ScoreboardUI_GetTeamResources(teamObject.teamNumber)), "")
    ...
    if table.icount(teamObject.playerList) ~= teamObject.numPlayers then
        self:ResizePlayerList(teamObject.playerList, teamObject.numPlayers, teamObject.teamGUIItem)
    end
    ...
    for index, player in ipairs(teamObject.playerList) do
        ...
        playerObject.isDead = playerObject.playerStatus == ResolveLocalizedString("STATUS_DEAD")
        ...
        if playerObject.playerSkillCapped then
            sumPlayerSkill = sumPlayerSkill + playerObject.playerSkillCapped
    ...
    -- Team skill avg icon
    if teamObject.isPlayingTeam and teamObject.teamSkillGUIItem.sumPlayerSkill ~= sumPlayerSkill then
        if teamObject.numPlayers > 0 then
            teamObject.teamSkillGUIItem.sumPlayerSkill = sumPlayerSkill
            local avgSkill = numPlayerSkill < 1 and 0 or sumPlayerSkill / numPlayerSkill
            avgSkill = (sumPlayerSkill + avgSkill * (teamObject.numPlayers - numPlayerSkill)) / teamObject.numPlayers
            ...
            teamObject.teamSkillGUIItem:SetIsVisible(false)
        end
    end

    -- Enable/Disable topbar
    if teamObject.teamNumber == kTeam1Index then
        self.team1topbar.background:SetIsVisible(teamObject.isVisibleTeam)
    elseif teamObject.teamNumber == kTeam2Index then
        self.team2topbar.background:SetIsVisible(teamObject.isVisibleTeam)
    end

    -- EalUpdate
    if throttlePerformUpdates then
        self:UpdateTeam_EalIcons(teamObject)
    end
end

function GUIScoreboard:ResizePlayerList(playerList, numPlayers, teamGUIItem)
    PROFILE("GUIScoreboard:ResizePlayerList")
...
function GUIScoreboard:CreatePlayerItem()
    PROFILE("GUIScoreboard:CreatePlayerItem")
...
local function HandlePlayerVoiceClicked(self)
    PROFILE("GUIScoreboard:HandlePlayerVoiceClicked")
    if MouseTracker_GetIsVisible() then
        local mouseX, mouseY = Client.GetCursorPosScreen()
...
local function HandlePlayerTextClicked(self)
    PROFILE("GUIScoreboard:HandlePlayerTextClicked")
    if MouseTracker_GetIsVisible() then
        local mouseX, mouseY = Client.GetCursorPosScreen()
        for t = 1, #self.teams do
            local playerList = self.teams[t]["PlayerList"]
...
function GUIScoreboard:SetIsVisible(state)
    PROFILE("GUIScoreboard:SetIsVisible")
...
function GUIScoreboard:CreateEALGraphicHeader(team, color, logoTexture, logoCoords, logoSizeX, logoSizeY)
    PROFILE("GUIScoreboard:CreateEALGraphicHeader")
    local item = {}
