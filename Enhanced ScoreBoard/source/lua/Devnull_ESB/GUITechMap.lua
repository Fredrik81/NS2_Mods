...
function GUITechMap:Initialize()
    ...
    print("Initialize: " .. tostring(self.teamType))
    print("#techMap: " .. tostring(#techMap))
    print("kTechMaps: " .. tostring(kTechMaps))
    for i = 1, #techMap do

        local entry = techMap[i]
        print("Entry (" .. tostring(i) .. "): " .. tostring(entry[1]) .. " = " .. Locale.ResolveString(LookupTechData(entry[1], kTechDataDisplayName, "unknown")))
...
function GUITechMap:Update(deltaTime)
    ...
    if teamType ~= self.teamType then
        print("teamType: " .. tostring(teamType))
        ClearTechTree()
        self:Uninitialize()
        self:Initialize()
    ...
    if showMap then
        ...
        print("TeamType: " .. tostring(self.teamType))
        print("player:GetTeamNumber(): " .. tostring(player:GetTeamNumber()))
        --local tree = GetGamerules():GetTeam(self.teamType)
        local techTree = GetTechTree() --here is problem
        ...
        if techTree then
            ...
            print("NOOOOO techTree!!!!")
        end

        if techTree then
            print("=============================================")
            print("TechIcons: " .. tostring(#self.techIcons))
            print(tostring(techTree))
            for i = 1, #self.techIcons do
                ...
                print("techNode: " .. tostring(techNode))
                if techNode then --problem here
                    print("TechID (" .. tostring(techId) .. "): " .. Locale.ResolveString(LookupTechData(techId, kTechDataDisplayName, "unknown")))
                    if techId == kTechId.InfantryPortal then
                        print("Tech IP: ")
                        print("- GetIsPassive : " .. tostring(techNode:GetIsPassive()))
                        print("- GetHasTech   : " .. tostring(techNode:GetHasTech()))
                        print("- GetAvailable : " .. tostring(techNode:GetAvailable()))
                        print("- GetResearched: " .. tostring(techNode:GetResearched()))
                    end
