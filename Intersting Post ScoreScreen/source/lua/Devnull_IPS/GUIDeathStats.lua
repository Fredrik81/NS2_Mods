...
class "GUIDeathStats"(GUIAnimatedScript)

StatsUIVisible = false
...
function GUIDeathStats:UpdateSizeOfUI()
...
function GUIDeathStats:OnResolutionChanged(_, _, _, _)
	self:Uninitialize()
	self:Initialize()
end

function GUIDeathStats:Initialize()
...
function GUIDeathStats:AddRow(leftText, rightText)
...
function GUIDeathStats:Reset()
	GUIAnimatedScript.Reset(self)
end

function GUIDeathStats:Update(deltaTime)
	...
	local visState = (self.requestVisible or visible and GetAdvancedOption("deathstats") == 2) and not PlayerUI_IsOverhead()
	...
	-- Lazy mode: Engaged
	if not visible or GetAdvancedOption("deathstats") < 2 or binding == "None" then
	...
	if EndStatsVisible and visState then
		self.titleBackground:SetIsVisible(false)
		self.actionIconGUI:Hide()
	end
end

function GUIDeathStats:SendKeyEvent(key, down)
	-- Force show when request menu is open
	local player = Client.GetLocalPlayer()
	local teamNumber = player and player:GetTeamNumber()
	if GetIsBinding(key, "RequestMenu") and GetAdvancedOption("deathstats") > 0 and not EndStatsVisible and (player and teamNumber == kTeam1Index or teamNumber == kTeam2Index) and not ChatUI_EnteringChatMessage() and not MainMenu_GetIsOpened() and not PlayerUI_IsOverhead() then
...
function GUIDeathStats:Uninitialize()
...
function GUIDeathStats:ResetTableBackground()
...
function GUIDeathStats:SetStats()
	...
	if statsTable ~= nil then
		--print("Client Dump: " .. dump(statsTable))
		self:AddRow("Last Life Accuracy", round(statsTable.lastAcc, 0) .. "%")
		if statsTable.lastAccOnos > -1 then
			self:AddRow("Without Onos Hits", round(statsTable.lastAccOnos, 0) .. "%")
		end
		self:AddRow("Player Damage", round(statsTable.pdmg, 0))
		self:AddRow("Structure Damage", round(statsTable.sdmg, 0))
		if statsTable.kills > 0 then
			self:AddRow("Kills", round(statsTable.kills, 0))
		end
		if statsTable.medsReceived or statsTable.parasites or statsTable.marineRtDamage or statsTable.alienRtDamage then
			self:AddRow()
			if statsTable.medsReceived and statsTable.medsReceived > 0 then
				self:AddRow("Meds Received", round(statsTable.medsReceived, 0))
			end
			if statsTable.parasites and statsTable.parasites > 0 then
				self:AddRow("Parasites", round(statsTable.parasites, 0))
			end
			if statsTable.marineRtDamage and statsTable.marineRtDamage > 0 then
				self:AddRow("RT Damage", round(statsTable.marineRtDamage, 0))
			end
			if statsTable.alienRtDamage and statsTable.alienRtDamage > 0 then
				self:AddRow("RT Damage", round(statsTable.alienRtDamage, 0))
			end
		end

		self:AddRow()
		self:AddRow("Current Accuracy", round(statsTable.currentAcc, 0) .. "%")
		if statsTable.currentAccOnos > -1 then
			self:AddRow("Without Onos Hits", round(statsTable.currentAccOnos, 0) .. "%")
		end

		if statsTable.aliveFor then
			self:AddRow()
			self:AddRow("Alive For", string.format("%d:%02d", math.floor(statsTable.aliveFor), math.floor((statsTable.aliveFor % 1) * 60)))
			--self:AddRow("Alive For", string.format("%d:%02d", math.floor(statsTable.aliveFor), math.floor(math.mod(statsTable.aliveFor, 60))))
		end
	end
end
...
local function CHUDGetStatsString(message)
	if message then
		...
		local fadeOutFunc = function()
			gStatsUI.titleBackground:FadeOut(2, "CHUD_DEATHSTATS", AnimateLinear)
		end
		local pauseFunc = function()
			gStatsUI.titleBackground:Pause(6, "CHUD_DEATHSTATS", nil, fadeOutFunc)
		end
...
Client.HookNetworkMessage("DeathStats", CHUDGetStatsString)
