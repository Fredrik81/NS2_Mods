...
function Alien:ProcessBuyAction(techIds)
    ...
    if gameRules:GetGameStarted() or gameRules:GetWarmUpActive() then
        ...
        local trace = Shared.TraceCapsule(position + Vector(0, eggExtents.y + 0.2, 0), position + Vector(0, -4.5, 0),
            math.max(eggExtents.x, eggExtents.z), eggExtents.y, CollisionRep.Move, PhysicsMask.AllButPCs,
            EntityFilterAll())
        ...
        if not evolve_blocked then
            ...
            if not GetHasRoomForCapsule(newAlienExtents + spawnBufferExtents,
                ...
                for _ = 1, 100 do
                    ...
                    roomAfter = GetRandomSpawnForCapsule(newAlienExtents.y,
                        math.max(newAlienExtents.x, newAlienExtents.z), self:GetModelOrigin(), 0.5, 5,
