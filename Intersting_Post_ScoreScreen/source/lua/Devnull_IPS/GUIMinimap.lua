...
function MapCheckStartStopTimer(mode, comment)
    ...
    if mode == 2 and mapCheckFlag == true then
        local mapCheckTime = Shared.GetTime() - mapCheckStart
        mapCheckFlag = false
        ...
        if mapCheckTime > 0 and (Client.GetLocalPlayer():GetTeamNumber() == 1 or Client.GetLocalPlayer():GetTeamNumber() == 2) then
            ...
            Client.SendNetworkMessage("mapCheckTime", {
                mapCheckTime = mapCheckTime,
                teamNumber = Client.GetLocalPlayer():GetTeamNumber()
            }, true)
        end
    end
end
...
function GUIMinimap:ShowMap(showMap)
    if showMap then
        MapCheckStartStopTimer(1, 'minimap:showmap true')
	else
        MapCheckStartStopTimer(2, 'minimap:showmap false')
    end
