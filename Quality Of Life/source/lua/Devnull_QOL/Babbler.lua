...
function Babbler:OnInitialized()
	...
	if Server then
		...
		self.targetSelector = TargetSelector():Init(self, kTargetSearchRange, true, {kAlienStaticTargets, kAlienMobileTargets})
...
if Server then
	...
	function Babbler:Move(targetPos, deltaTime)
		...
		if self.cursor and self.cursor.index + 5 <= #self.cursor.points and self.timeNextMoveJump < now then
			local deviation = 0.15 + math.random() / 6
			local targetOrigin = self.cursor.points[self.cursor.index + 2]
			local targetVelocity = Vector(math.random() - 0.5 < 0 and deviation or -deviation, math.random() / 1.2, math.random() - 0.5 < 0 and deviation or -deviation)
	...
	function Babbler:BabblerOffMap()
		...
		if babblerOffMap then
			...
			if #RTs > 0 then
				...
				for _ = 1, 10 do
					inMapOrig = GetRandomSpawnForCapsule(1, 1, RTs[1]:GetOrigin() + Vector(0, 1, 0), 1, 6, EntityFilterAll())
					if inMapOrig then
						break
					end
				end
	...
	function Babbler:UpdateAttack(force)
		...
		-- Adjust the jump
		if target and self.kNextUpdateAttack and self.kStopImpulseDone < 3 and 0.2 < timeSinceLastAttack then
		...
		if force or (self.moveType == kBabblerMoveType.Attack and self:GetIsOnGround() and now > self.kNextUpdateAttack and not self.babblerOffMap) then
				...
				if self.timeLastJump + 0.5 < Shared.GetTime() then
					...
					if targetPosition and (not target or followTarget) then
						...
						if done or (self:GetOrigin() - targetPosition):GetLengthXZ() < 0.5 then
							...
							self:SetMoveType(target and kBabblerMoveType.Attack or kBabblerMoveType.None, target, targetPosition)
	...
	local function BabblerSetLinearDampingCallback(self)
		...
		if self.physicsBody and not self.clinged and (self:GetIsOnGround() or lifeSpan >= kFinalDampingAfterDelay) then
	...
	local function OnMoveTypeChanged(self)
		...
		if effectName and (self.moveType ~= kBabblerMoveType.Attack) then
