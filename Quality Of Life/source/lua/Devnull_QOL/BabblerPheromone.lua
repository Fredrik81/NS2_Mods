...
if Server then
	...
	local function CanGetAttach(self, entity)
		...
		local owner = self:GetOwner()
		if entity ~= owner and HasMixin(entity, "BabblerOwner") and entity:GetBabblerCount() >= entity:GetMaxBabblers() then
			return false
		end
	...
	function BabblerPheromone:ProcessHit(entity)
		...
		local isValidHit = entity and (entity:isa("PowerPoint") and entity:GetBuiltFraction() >= 0.009) or (entity and (GetAreEnemies(self, entity) or HasMixin(entity, "BabblerCling")) and HasMixin(entity, "Live") and entity:GetIsAlive())
