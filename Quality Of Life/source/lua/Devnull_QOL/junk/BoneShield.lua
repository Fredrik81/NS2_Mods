...
if Client then
    Script.Load("lua/MPBalance/GUIBoneShieldDisplay.lua")
end

-- TODO(Salads): BoneShield new cinimatic (waiting), Player.screenEffects.boneShieldVision
...
local networkVars =
{
    hitPoints = "private float (0 to 1000 by 0.01)", -- TODO(Salads): Tweak float resolution
...
function BoneShield:OnCreate()
    ...
    self.hitPoints = kBoneShieldHitpoints
    self.lastTimeDeactivated = 0
    self.hitPointsRechargeStartTime = 0
    self.holsterTime = 0

    if Client then
        self.lastHitPoints = kBoneShieldHitpoints
    end
    ...
    function BoneShield:OnInitialized()
        ...
        if player then
            ...
            if player:GetIsLocalPlayer() and
                ...
                self.guiObj:AlignBottom()
                self.guiObj:SetY(-200)
    ...
    function BoneShield:OnUpdateRender()
        ...
        if self.guiObj then
            local shouldBeVisible =
                    localPlayer and localPlayer:isa("Onos") and
                    self.active and
                    not HelpScreen_GetHelpScreen():GetIsBeingDisplayed() and
                    not GetMainMenu():GetVisible()
...
function BoneShield:GetCanUseBoneShield(player)
    ...
    return
            ...
            and self.hitPoints > (kBoneShieldHitpoints * kBoneShieldMinimumStartHitpointsFactor)
...
function BoneShield:ProcessMoveOnWeapon(player, input)
    ...
    if self.primaryAttacking then
        if self.hitPoints <= kEpsilon or not player:GetIsAlive() then
            self:OnBroken()
        end
    elseif now > self.hitPointsRechargeStartTime then
        ...
        if self.holsterTime > 0 then
            hitpointsToRecover = (now - self.holsterTime) * kBoneShieldHitpointsRegenRate
        else
            hitpointsToRecover = kBoneShieldHitpointsRegenRate * input.time
        end

        self.hitPoints = Clamp(self.hitPoints + hitpointsToRecover, 0, kBoneShieldHitpoints)
    end
