...
local kFairStartTexture = PrecacheAsset("ui/FairStart.dds")
local FairStartShouldClose = nil
...
local function GUICreateFairStartPing(team)
    ...
    if team == 1 then
        image:SetTexturePixelCoordinates(0, 0, 128, 128)
    else
        image:SetTexturePixelCoordinates(0, 128, 128, 256)
    end
...
function GUIMinimap:Update(deltaTime)
    ...
    if GetGameInfoEntity():GetState() == kGameState.Countdown and player:GetIsOnPlayingTeam() then
        ...
        if (CurrentCountDown < (kCountDownLength - 0.5) and CurrentCountDown > 1.5) and not player:isa("Commander") then
            if not FairStartShouldClose then
        ...
        elseif FairStartShouldClose then
            local minimapFrameScript = ClientUI.GetScript("GUIMinimapFrame")
            if minimapFrameScript then
            ...
            FairStartShouldClose = nil
        end
    elseif FairStartShouldClose and GetGameInfoEntity():GetState() ~= kGameState.Countdown then
        local minimapFrameScript = ClientUI.GetScript("GUIMinimapFrame")
