...
if Server then
        ...
        for _, player in ientitylist(Shared.GetEntitiesWithClassname("Player")) do      --FIXME This is setting enter times to invalid ranges (because round data is reset..thus, not releative to game-start)
            if player.ResetScores and player.client then
                ...
                if player:isa("Commander") then
                    Log("Set Commander[%s] game-start Enter time", player:GetId())
                    self.playerRanking:SetCommanderEntranceTime( player, player:GetTeamNumber() )
                end
                
            end
        end

        GetBotAccuracyTracker():Reset()
    ...
    function NS2Gamerules:OnCommanderLogin(commandStructure, newCommander)
        local teamNumber = commandStructure:GetTeamNumber()
        ...
        if teamInfo:GetLastCommIsBot() then
            for i = 1, #gServerBots do
                ...
                if bot and bot:GetPlayer():GetTeamNumber() == teamNumber then
                    bot:Disconnect()
                    break
                end
            end
        ...
        self.botTeamController:EnsureRebalanceCallback()
    ...
    NS2Gamerules.resetProtectedEntities = {
        "GameInfo",
        "MapBlip",
        "NS2Gamerules",
        "PlayerInfoEntity",
